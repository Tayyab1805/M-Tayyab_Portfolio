import { Linkedin, Mail, Phone, Heart } from "lucide-react";

const KaggleLogo = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" xmlns="http://www.w3.org/2000/svg">
    <path d="M18.825 23.859c-.022.092-.117.141-.281.141h-3.139c-.187 0-.351-.082-.492-.248l-5.178-6.589-1.448 1.374v5.111c0 .235-.117.352-.351.352H5.505c-.236 0-.354-.117-.354-.352V.353c0-.233.118-.353.354-.353h2.431c.234 0 .351.12.351.353v14.343l6.203-6.272c.165-.165.33-.246.495-.246h3.239c.144 0 .236.06.281.18.046.149.034.255-.036.315l-6.555 6.344 6.836 8.507c.095.104.117.208.075.285z"/>
  </svg>
);

export const Footer = () => {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="py-8 bg-primary text-primary-foreground">
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          {/* Logo */}
          <a
            href="#home"
            className="font-display text-2xl font-bold hover:text-accent transition-colors"
          >
            MT<span className="text-accent">.</span>
          </a>

          {/* Social Links */}
          <div className="flex items-center gap-4">
            <a
              href="mailto:tayyabtayyabe@gmail.com"
              className="p-2 rounded-lg hover:bg-primary-foreground/10 transition-colors"
              aria-label="Email"
            >
              <Mail className="w-5 h-5" />
            </a>
            <a
              href="tel:+923205722442"
              className="p-2 rounded-lg hover:bg-primary-foreground/10 transition-colors"
              aria-label="Phone"
            >
              <Phone className="w-5 h-5" />
            </a>
            <a
              href="https://www.linkedin.com/in/tayyab1805"
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 rounded-lg hover:bg-primary-foreground/10 transition-colors"
              aria-label="LinkedIn"
            >
              <Linkedin className="w-5 h-5" />
            </a>
            <a
              href="https://www.kaggle.com/tayyab1805"
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 rounded-lg hover:bg-primary-foreground/10 transition-colors flex items-center gap-2"
              aria-label="Kaggle Profile"
            >
              <KaggleLogo />
              <span className="text-sm font-medium hidden sm:inline">Kaggle</span>
            </a>
          </div>

          {/* Copyright */}
          <p className="text-sm text-primary-foreground/70 flex items-center gap-1">
            © {currentYear} Muhammad Tayyab. Made with <Heart className="w-4 h-4 text-accent fill-current" />
          </p>
        </div>
      </div>
    </footer>
  );
};
